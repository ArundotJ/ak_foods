        ...
        if (selectedCustomer != null) {
          ...
          for (var i = 0; i < currentStockItems.length; i++) {
            ...
            final String invoiceProduct = await DataBaseManager()
                .updateQueryFromSQL(
                    "Insert into Invoice_Product(InvoiceID, ProductID, BatchNo,Qty, SalesRate, DiscountPer, Discount, CGSTPer, CGSTAmt, SGSTPer, SGSTAmt, IGSTPer, IGSTAmt,MfgDate,ExpiryDate, TotalAmount, PurchaseRate, Margin,MRP,Barcode,SubUnitQty) VALUES ('$newInvoiceID','${pro.productId}','First','${pro.quantityDelivered}','${pro.rate}','0','0','0','0','0','0','0','0','${DateTime.now().getDateOnly()}','${DateTime.now().getDateOnly()}','${quantity * pro.rate}', '${pro.rate}', '0', '${pro.rate}','0','${pro.defaultQuantityReturn}')");
            final String vcDetails2 = await DataBaseManager().updateQueryFromSQL(
                "Insert into Voucher_OtherDetails2(VoucherID,InvoiceNo,CustomerID,customerName,ProductID,ProductName,Rate,QtyDelivered,QtyReturned,NetQty,TotalAmount,SalesmanID,SalesmanName) VALUES ('${pro.voicherID}','$newInvoiceID', '${selectedCustomer!.id}','${selectedCustomer!.name.trim()}','${pro.productId}','${pro.productName.trim()}', '${pro.rate}','${pro.quantityDelivered}','${pro.defaultQuantityReturn}','$quantity','${pro.rate * pro.quantityDelivered}','$newInvoiceID', '${widget.user.name.trim()}')");
